...
final class IntelligenceBehaviorTests: XCTestCase {
    ...
    func testCurriculumQuantificationScoresKnownNodeAndFallsBackOnUnknown() async throws {
        ...
        XCTAssertEqual(result.dimensions.examValue, 0.09 * 0.5 + 0.9 * 0.5, accuracy: 0.001)
        XCTAssertGreaterThan(result.dimensions.knowledgeValue, 0)
        XCTAssertLessThanOrEqual(result.dimensions.knowledgeValue, 1)
        XCTAssertGreaterThan(result.dimensions.reviewPriority, 0)
        XCTAssertLessThanOrEqual(result.dimensions.reviewPriority, 1)
        XCTAssertEqual(result.inputContentRevision, 3)
