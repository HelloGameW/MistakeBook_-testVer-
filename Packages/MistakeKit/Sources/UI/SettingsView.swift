            ...
            Form {
                ...
                Section("OCR") {
                    ...
                    if ocrProvider == .modelAPI {
                        APIConfigurationFields(title: "OCR 模型", baseURL: $ocrBaseURL, endpoint: $ocrEndpoint, model: $ocrModel)
                    }
                ...
                Section("错因分析") {
                    ...
                    if analysisProvider == .modelAPI {
                        APIConfigurationFields(title: "错因模型", baseURL: $analysisBaseURL, endpoint: $analysisEndpoint, model: $analysisModel)
                    }
                ...
                Section("错题价值量化") {
                    ...
                    if valueProvider == .modelAPI {
                        APIConfigurationFields(title: "价值模型", baseURL: $valueBaseURL, endpoint: $valueEndpoint, model: $valueModel)
                    }
        ...
        .task {
            do {
                let query = RecordQuery(text: "", subjectID: nil, taxonomyNodeID: nil, includeDescendants: true,
                                        reviewStates: [], reviewRequiredOnly: false, includeDeleted: false, sort: .updatedNewest)
