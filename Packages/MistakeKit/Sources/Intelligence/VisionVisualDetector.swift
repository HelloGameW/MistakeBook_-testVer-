...
enum VisionVisualDetector {
    ...
    private enum VisualKind: Equatable {
        ...
        var code: String {
            switch self {
            case .image: return "vision.visual.image"
            case .table: return "vision.visual.table"
            case .formula: return "vision.visual.formula"
            }
        }
    ...
    static func build(assetID: UUID,
                      rectangles: [VisionVisualObservation],
                      lines: [OCRLine]) throws -> VisualRegionBuildResult {
        ...
        for observation in rectangles {
            guard observation.confidence >= 0.25,
                  ...
                  rect.width * rect.height <= 0.85 else {
                continue
            }
            let inside = lines.filter { Self.center(of: $0.normalizedRect, isInside: rect) }
            let kind: VisualKind = Self.looksLikeTable(inside) ? .table : .image
            if !Self.hasStrongDuplicate(of: rect, in: detected) {
                detected.append(DetectedRegion(region: SourceRegion(id: UUID(), assetID: assetID,
                                                                     normalizedRect: rect, purpose: .diagram,
                                                                     isUserConfirmed: false), kind: kind))
            }
        }

        if let tableLines = Self.tableLines(lines),
           !detected.contains(where: { if case .table = $0.kind { return true } else { return false } }),
           let rect = try? Self.boundingRect(of: tableLines) {
            detected.append(DetectedRegion(region: SourceRegion(id: UUID(), assetID: assetID,
                                                                 normalizedRect: rect, purpose: .diagram,
                                                                 isUserConfirmed: false), kind: .table))
        }

        for line in lines where Self.looksLikeFormula(line.rawText) {
            guard let rect = try? Self.expandedRect(line.normalizedRect, padding: 0.006) else { continue }
            if !Self.hasStrongDuplicate(of: rect, in: detected, matching: .formula) {
                detected.append(DetectedRegion(region: SourceRegion(id: UUID(), assetID: assetID,
                                                                     normalizedRect: rect, purpose: .diagram,
                                                                     isUserConfirmed: false), kind: .formula))
            }
        }

        var lineRegionIDs: [UUID: UUID] = [:]
        for line in lines {
            let matching = detected.filter { Self.center(of: line.normalizedRect, isInside: $0.region.normalizedRect) }
            if let best = matching.min(by: { Self.area($0.region.normalizedRect) < Self.area($1.region.normalizedRect) }) {
                lineRegionIDs[line.id] = best.region.id
            }
        }

        let warnings = detected.map {
            ServiceWarning(code: $0.kind.code, message: $0.kind.message, regionID: $0.region.id)
        }
        return VisualRegionBuildResult(regions: detected.map(\.region), lineRegionIDs: lineRegionIDs,
                                       warnings: warnings)
    }
    ...
    private static func looksLikeFormula(_ text: String) -> Bool {
        ...
        let hasMathSymbol = trimmed.unicodeScalars.contains { mathSymbols.contains($0) }
        let hasArithmeticPattern = trimmed.range(of: #"[A-Za-z0-9)]\s*[+\-*/=]\s*[A-Za-z0-9(]"#, options: .regularExpression) != nil
        let hasGreek = trimmed.unicodeScalars.contains { scalar in
            (0x0370...0x03FF).contains(Int(scalar.value))
        }
        let hasOperand = trimmed.range(of: #"[A-Za-z0-9]"#, options: .regularExpression) != nil
        return hasOperand && (hasMathSymbol || hasArithmeticPattern || hasGreek)
    }

    private static func looksLikeTable(_ lines: [OCRLine]) -> Bool {
        guard let tableLines = tableLines(lines) else { return false }
        return tableLines.count >= 4
    }

    private static func tableLines(_ lines: [OCRLine]) -> [OCRLine]? {
        guard lines.count >= 4 else { return nil }
        let rows = rowGroups(lines)
        let populatedRows = rows.filter { distinctColumnCount($0) >= 2 }
        guard populatedRows.count >= 2 else { return nil }
        return populatedRows.flatMap { $0 }
    }

    private static func rowGroups(_ lines: [OCRLine]) -> [[OCRLine]] {
        var groups: [[OCRLine]] = []
        for line in lines.sorted(by: { center(of: $0.normalizedRect).y < center(of: $1.normalizedRect).y }) {
            let lineCenter = center(of: line.normalizedRect).y
            var assigned = false
            for index in groups.indices {
                let rowCenter = groups[index].map { center(of: $0.normalizedRect).y }.reduce(0, +)
                    / Double(groups[index].count)
                if abs(rowCenter - lineCenter) <= max(0.012, line.normalizedRect.height * 1.5) {
                    groups[index].append(line)
                    assigned = true
                    break
                }
            }
            if !assigned { groups.append([line]) }
        }
        return groups
    }

    private static func distinctColumnCount(_ lines: [OCRLine]) -> Int {
        var columns: [Double] = []
        for line in lines.sorted(by: { $0.normalizedRect.x < $1.normalizedRect.x }) {
            let x = line.normalizedRect.x
            if columns.allSatisfy({ abs($0 - x) > 0.045 }) { columns.append(x) }
        }
    ...
    private static func hasStrongDuplicate(of rect: NormalizedRect,
        ...
        detected.contains { item in
            ...
            return overlap(rect, item.region.normalizedRect) >= 0.68
        }
    }
